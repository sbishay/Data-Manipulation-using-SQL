...
---1.List all the states in which we have customers who have bought cellphones from 2005 till today.
  ...
  SELECT STATE,
		...
		FROM DIM_LOCATION
			INNER JOIN
		FACT_TRANSACTIONS ON DIM_LOCATION.IDLOCATION = FACT_TRANSACTIONS.IDLOCATION
...
SELECT 
		...
		STATE,
		MANUFACTURER_NAME,
		COUNT(STATE) AS [COUNT OF NUMBERS]
		FROM DIM_LOCATION
			INNER JOIN 
		FACT_TRANSACTIONS ON DIM_LOCATION.IDLocation = FACT_TRANSACTIONS.IDLocation
			INNER JOIN
		...
		ORDER BY [COUNT OF NUMBERS] DESC
...
SELECT		
		...
		ZIPCODE,
		Model_NAME,
		COUNT(ZIPCODE) AS [NUMBER OF TRANSACTIONS]
		FROM DIM_LOCATION
			INNER JOIN 
		FACT_TRANSACTIONS ON DIM_LOCATION.IDLocation = FACT_TRANSACTIONS.IDLocation
			INNER JOIN
...
SELECT 
		...
		MANUFACTURER_NAME,
		MODEL_NAME,
		MIN(UNIT_PRICE) AS [CHEAPEST PRICE]
		FROM DIM_MODEL
			INNER JOIN
		DIM_MANUFACTURER ON DIM_MODEL.IDMANUFACTURER = DIM_MANUFACTURER.IDMANUFACTURER
		GROUP BY MODEL_NAME,MANUFACTURER_NAME
		ORDER BY [CHEAPEST PRICE] ASC
...
SELECT 
		P2.Model_Name AS [MODEL],
		AVG(P1.TotalPrice) AS [AVG. PRICE]
		FROM
        FACT_TRANSACTIONS P1
		INNER JOIN
		DIM_MODEL P2    ON   P1.IDMODEL=P2.IDMODEL
		INNER JOIN
		DIM_MANUFACTURER P3   ON   P2.IDMANUFACTURER=P3.IDMANUFACTURER
         WHERE manufacturer_name IN 
		 (SELECT Manufacturer_Name AS [MANUFACTURER]
                   FROM
                   (SELECT  TOP 5 t3.manufacturer_name,SUM(t1.quantity) AS [QUANTITY],AVG(TotalPrice) AS [avg_price]
                      FROM
		              ...
		              ORDER BY avg_price DESC) AS M1)
		   GROUP BY P2.Model_Name


---6.List the names of the customers and average amount spent in 2009,where the average is higher than 500.

SELECT 
		T1.CUSTOMER_NAME,
		YEAR(T2.DATE) AS [YEAR],
		AVG(T2.TOTALPRICE) AS [AVERAGE SPENT]
		FROM DIM_CUSTOMER T1
			LEFT JOIN
		FACT_TRANSACTIONS T2 ON T1.IDCUSTOMER = T2.IDCUSTOMER
...
INTERSECT
    ...
    FROM  FACT_TRANSACTIONS T1
		 ...
		 GROUP BY T2.MODEL_NAME, YEAR(T1.DATE)
		 ORDER BY QUANTITY DESC) AS M3
...
SELECT * FROM
	(SELECT TOP 1 Manufacturer_Name as [manufacturer],years,sales
	FROM
	(SELECT TOP 2 Manufacturer_Name,SUM(TOTALPRICE) as [sales],year(date) AS [YEARS]
	FROM FACT_TRANSACTIONS T1
				   INNER JOIN
				   DIM_MODEL T2   ON T1.IDModel=T2.IDModel
				   INNER JOIN
				   ...
				   ORDER by SALES) AS M3
				   
	UNION 
	
	SELECT * FROM 
	(SELECT TOP 1 MANUFACTURER_NAME,YEAR,SALES
	FROM
	(SELECT top 2 Manufacturer_Name,year(t1.Date) as [year],SUM(TOTALPRICE) as [sales]
	FROM FACT_TRANSACTIONS T1
				   INNER JOIN
				   DIM_MODEL T2   ON T1.IDModel=T2.IDModel
				   INNER JOIN
				   ...
				   group by Manufacturer_Name,year(date)
				   order by sales DESC) AS M2
				   ORDER BY sales) AS M4
